...
impl Database {
    ...
    pub fn stat<'txn>(&self, txn: &'txn Txn<'txn>) -> MdbResult<ffi::MDB_stat> {
        let native = txn.get_inner_txn();
        let mut tmp: ffi::MDB_stat = unsafe { std::mem::zeroed() };
        lift_mdb!(unsafe { ffi::mdb_stat(native.handle, self.handle, &mut tmp)}, tmp)
    }

    fn get_value<'txn, V: FromMdbValue + 'txn>(&self, key: &ToMdbValue, txn: &'txn Txn<'txn>) -> MdbResult<V> {
        let native = txn.get_inner_txn();
        let mut key_val = key.to_mdb_value();
        unsafe {
            let mut data_val: MdbValue = std::mem::zeroed();
            try_mdb!(ffi::mdb_get(native.handle, self.handle, &mut key_val.value, &mut data_val.value));
            Ok(FromMdbValue::from_mdb_value(&data_val))
        }
    }

    /// Retrieves a value by key. In case of DbAllowDups it will be the first value
    pub fn get<'txn, V: FromMdbValue + 'txn>(&self, key: &ToMdbValue, txn: &'txn Txn<'txn>) -> MdbResult<V> {
        let native = txn.get_inner_txn();

        assert_state_eq!(txn, native.state, TransactionState::Normal);
        self.get_value(key, txn)
    }

    fn set_value<'txn>(&self, key: &ToMdbValue, value: &ToMdbValue, txn: &'txn Txn<'txn>) -> MdbResult<()> {
        let native = txn.get_inner_txn();
        self.set_value_with_flags(key, value, 0, txn)
    }

    fn set_value_with_flags<'txn>(&self, key: &ToMdbValue, value: &ToMdbValue, flags: c_uint, txn: &'txn Txn<'txn>) -> MdbResult<()> {
        let native = txn.get_inner_txn();
        unsafe {
            let mut key_val = key.to_mdb_value();
            let mut data_val = value.to_mdb_value();

            lift_mdb!(ffi::mdb_put(native.handle, self.handle, &mut key_val.value, &mut data_val.value, flags))
        }
    }

    /// Sets value for key. In case of DbAllowDups it will add a new item
    pub fn set<'txn>(&self, key: &ToMdbValue, value: &ToMdbValue, txn: &'txn Txn<'txn>) -> MdbResult<()> {
        let native = txn.get_inner_txn();
    ...
    pub fn append<'txn, K: ToMdbValue, V: ToMdbValue>(&self, key: &K, value: &V, txn: &'txn Txn<'txn>) -> MdbResult<()> {
        let native = txn.get_inner_txn();
        assert_state_eq!(txn, native.state, TransactionState::Normal);
    ...
    pub fn append_duplicate<'txn, K: ToMdbValue, V: ToMdbValue>(&self, key: &K, value: &V, txn: &'txn Txn<'txn>) -> MdbResult<()> {
        let native = txn.get_inner_txn();
        assert_state_eq!(txn, native.state, TransactionState::Normal);
    ...
    pub fn insert<'txn>(&self, key: &ToMdbValue, value: &ToMdbValue, txn: &'txn Txn<'txn>) -> MdbResult<()> {
        let native = txn.get_inner_txn();
        assert_state_eq!(txn, native.state, TransactionState::Normal);
        self.set_value_with_flags(key, value, ffi::MDB_NOOVERWRITE, txn)
    }

    fn del_value<'txn>(&self, key: &ToMdbValue, txn: &'txn Txn<'txn>) -> MdbResult<()> {
        let native = txn.get_inner_txn();
        unsafe {
            let mut key_val = key.to_mdb_value();
            lift_mdb!(ffi::mdb_del(native.handle, self.handle, &mut key_val.value, ptr::null_mut()))
        }
    }

    /// Deletes value for key.
    pub fn del<'txn>(&self, key: &ToMdbValue, txn: &'txn Txn<'txn>) -> MdbResult<()> {
        let native = txn.get_inner_txn();
        assert_state_eq!(txn, native.state, TransactionState::Normal);
    ...
    pub fn del_item<'txn>(&self, key: &ToMdbValue, data: &ToMdbValue, txn: &'txn Txn<'txn>) -> MdbResult<()> {
        let native = txn.get_inner_txn();
        assert_state_eq!(txn, native.state, TransactionState::Normal);
        unsafe {
            let mut key_val = key.to_mdb_value();
            ...
            lift_mdb!(ffi::mdb_del(native.handle, self.handle, &mut key_val.value, &mut data_val.value))
        }
    }

    /// Returns a new cursor
    pub fn new_cursor<'txn>(&self, txn: &'txn Txn<'txn>) -> MdbResult<Cursor<'txn>> {
        let native = txn.get_inner_txn();
        Cursor::new(&native, self.handle)
    }

    /// Deletes current db, also moves it out
    pub fn del_db<'txn>(self, txn: &'txn Txn<'txn>) -> MdbResult<()> {
        let native = txn.get_inner_txn();
        assert_state_eq!(txn, native.state, TransactionState::Normal);
        unsafe {
            native.env.drop_db_from_cache(self.handle);
            lift_mdb!(ffi::mdb_drop(native.handle, self.handle, 1))
        }
    }

    /// Removes all key/values from db
    pub fn clear<'txn>(&self, txn: &'txn Txn<'txn>) -> MdbResult<()> {
        let native = txn.get_inner_txn();
        assert_state_eq!(txn, native.state, TransactionState::Normal);
        unsafe {
            lift_mdb!(ffi::mdb_drop(native.handle, self.handle, 0))
        }
    }

    /// Returns an iterator for all values in database
    pub fn iter<'txn>(&self, txn: &'txn Txn<'txn>) -> MdbResult<CursorIterator<'txn, CursorIter>> {
        let native = txn.get_inner_txn();
        self.new_cursor(txn)
    ...
    pub fn keyrange_from<'c, K: ToMdbValue + 'c>(&'c self, start_key: &'c K, txn: &'c Txn<'c>) -> MdbResult<CursorIterator<'c, CursorFromKeyIter>> {
        let native = txn.get_inner_txn();
    ...
    pub fn keyrange_to<'c, K: ToMdbValue + 'c>(&'c self, end_key: &'c K, txn: &'c Txn<'c>) -> MdbResult<CursorIterator<'c, CursorToKeyIter>> {
        let native = txn.get_inner_txn();
        let cursor = try!(self.new_cursor(txn));
    ...
    pub fn keyrange_from_to<'c, K: ToMdbValue + 'c>(&'c self, start_key: &'c K, end_key: &'c K, txn: &'c Txn<'c>)
    ...
    pub fn keyrange<'c, K: ToMdbValue + 'c>(&'c self, start_key: &'c K, end_key: &'c K, txn: &'c Txn<'c>)
                               -> MdbResult<CursorIterator<'c, CursorKeyRangeIter>>
    {
        let native = txn.get_inner_txn();
        let cursor = try!(self.new_cursor(txn));
    ...
    pub fn item_iter<'c, 'db: 'c, K: ToMdbValue>(&'db self, key: &'c K, txn: &'c Txn<'c>) -> MdbResult<CursorIterator<'c, CursorItemIter<'c>>> {
        let native = txn.get_inner_txn();
        let cursor = try!(self.new_cursor(txn));
    ...
    pub fn set_compare<'txn>(&self, cmp_fn: extern "C" fn(*const MDB_val, *const MDB_val) -> c_int, txn: &'txn Txn<'txn>) -> MdbResult<()> {
        let native = txn.get_inner_txn();
        lift_mdb!(unsafe {
            ffi::mdb_set_compare(native.handle, self.handle, cmp_fn)
        })
    }
    ...
    /// Setting lasts for the lifetime of the underlying db handle.
    pub fn set_dupsort<'txn>(&self, cmp_fn: extern "C" fn(*const MDB_val, *const MDB_val) -> c_int, txn: &'txn Txn<'txn>) -> MdbResult<()> {
        let native = txn.get_inner_txn();

        lift_mdb!(unsafe {
            ffi::mdb_set_dupsort(native.handle, self.handle, cmp_fn)
        })
    }
}
